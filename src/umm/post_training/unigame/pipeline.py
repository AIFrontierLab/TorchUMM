...
def _build_launcher_prefix(torchrun_cfg: dict[str, Any], use_torchrun: bool) -> list[str]:
    ...
    # Prefer torchrun when available, fallback to python -m torch.distributed.run.
...
def run_unigame_train(cfg: dict[str, Any], config_path: str | None = None) -> None:
    backbone = str(cfg.get("backbone", "")).strip().lower()
    if backbone == "janus_pro":
        from umm.post_training.unigame.janus_pro.train import run_janus_pro_train

        run_janus_pro_train(cfg)
        return
    if backbone == "bagel":
        from umm.post_training.unigame.bagel.train import run_bagel_train

        run_bagel_train(cfg, config_path=config_path)
        return
    ...
    if not script and not module:
        raise ValueError("Training config requires non-empty `script` or `module`.")
    ...
    if module:
        ...
        if not script_path.exists():
            raise FileNotFoundError(f"UniGame training script not found: {script_path}")
