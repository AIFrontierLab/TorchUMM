...
# This script uses bash arrays/slicing; if launched with `sh`, re-run in bash.
...
if  [ ${DATASET} == "mme" ]; then
  ...
  if [ ! -f "${DIRNAME}/existence.txt" ]; then
    echo "MME evaluation outputs are incomplete: ${DIRNAME}/existence.txt not found."
    echo "Check prior eval.py errors and ensure ../../data/mme/MME_Benchmark_release_version is prepared."
    exit 1
  fi
