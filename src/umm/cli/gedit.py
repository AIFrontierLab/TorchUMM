"""CLI entry point for GEdit-Bench — thin subprocess wrapper.

Follows the same pattern as cli/wise.py:
reads config, builds subprocess command, calls eval/edit/gedit/run_gedit_eval.py.
"""
...
def run_gedit_eval_command(args: Any) -> int:
    ...
    script_path = repo_root / "eval" / "edit" / "gedit" / "run_gedit_eval.py"
    if not script_path.exists():
        raise FileNotFoundError(f"GEdit runner not found: {script_path}")
    ...
    print(f"[umm eval] running GEdit-Bench, backbone={backbone}")
