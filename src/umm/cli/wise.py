"""CLI entry point for WISE benchmark — thin subprocess wrapper.

Follows the same pattern as cli/dpg_bench.py:
reads config, builds subprocess command, calls eval/generation/wise/run_wise_eval.py.
"""
...
def run_wise_eval_command(args: Any) -> int:
    ...
    script_path = repo_root / "eval" / "generation" / "wise" / "run_wise_eval.py"
    if not script_path.exists():
        raise FileNotFoundError(f"WISE runner not found: {script_path}")
    ...
    print(f"[umm eval] running WISE benchmark, backbone={backbone}")
