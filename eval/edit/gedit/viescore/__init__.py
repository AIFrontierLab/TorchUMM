...
class VIEScore:
    def __init__(self, backbone="qwen25vl", task="t2i", key_path=None, model_path=None) -> None:
        self.task = task
        ...
        if self.backbone_name == "qwen25vl":
            ...
            self.model = Qwen25VL(model_path=model_path)
        else:
            raise NotImplementedError(f"backbone '{backbone}' not supported, only 'qwen25vl' is available")
...
if __name__ == "__main__":
    model = VIEScore(backbone="qwen25vl", task="t2i")
