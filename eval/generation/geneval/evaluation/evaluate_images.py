...
def _load_completed(outfile):
    """Load already-evaluated image paths from an existing partial results file."""
    done = set()
...
def main(args):
    ...
    # Resume: load already-evaluated paths
    done = _load_completed(args.outfile)
    if done:
        print(f"[geneval-eval] resuming: {len(done)} images already evaluated", file=sys.stderr)

    # Collect all (imagepath, metadata) pairs first for accurate progress bar
    tasks = []
    ...
    # Evaluate with incremental writes (append mode)
    new_count = 0
    with open(args.outfile, "a") as fp:
        for imagepath, metadata in tqdm(tasks):
            if imagepath in done:
                continue
    ...
    print(f"[geneval-eval] done: {new_count} new + {len(done)} resumed = {len(done) + new_count} total", file=sys.stderr)
